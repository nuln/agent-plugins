...
func newCodexSession(ctx context.Context, workDir, model, effort, mode, resumeID string, extraEnv []string) *codexSession {
...
func (cs *codexSession) Send(prompt string, images []agent.ImageAttachment, files []agent.FileAttachment) error {
	if len(files) > 0 {
		filePaths := saveFilesToDisk(cs.workDir, files)
		prompt = appendFileRefs(prompt, filePaths)
	}
	if !cs.alive.Load() {
	...
	slog.Debug("codexSession: launching", "args", redactArgs(args))
...
func (cs *codexSession) stageImages(prompt string, images []agent.ImageAttachment) (string, []string, error) {
	if len(images) == 0 {
...
func (cs *codexSession) handleEvent(raw map[string]any) {
	...
	case "turn.completed":
		...
		evt := agent.Event{Type: agent.EventResult, SessionID: cs.CurrentSessionID(), Done: true}
		select {
		case cs.events <- evt:
		case <-cs.ctx.Done():
			return
	...
	case "turn.failed":
		...
		evt := agent.Event{Type: agent.EventError, Error: fmt.Errorf("%s", errMsg)}
		select {
		case cs.events <- evt:
		case <-cs.ctx.Done():
			return
...
func (cs *codexSession) handleItemStarted(raw map[string]any) {
	...
	case "command_execution":
		...
		evt := agent.Event{Type: agent.EventToolUse, ToolName: "Bash", ToolInput: command}
		select {
		case cs.events <- evt:
		case <-cs.ctx.Done():
			return
	...
	case "function_call":
		...
		evt := agent.Event{Type: agent.EventToolUse, ToolName: name, ToolInput: args}
		select {
		case cs.events <- evt:
		case <-cs.ctx.Done():
			return
...
func (cs *codexSession) RespondPermission(_ string, _ agent.PermissionResult) error {
	return nil
}

func (cs *codexSession) Events() <-chan agent.Event {
	return cs.events
}
