...
func (p *LarkAccess) ReplyCard(ctx context.Context, rctx any, card *agent.Card) error {
...
func (p *InteractiveLarkAccess) SendCard(ctx context.Context, rctx any, card *agent.Card) error {
	rc, ok := rctx.(replyContext)
	if !ok {
		return fmt.Errorf("%s: invalid reply context type %T", p.tag(), rctx)
	}
...
// renderCardMap converts a Card into the Feishu Interactive Card map
// using the v1 format. Used both for message API (via renderCard) and
// callback responses (CardActionTriggerResponse).
func renderCardMap(card *agent.Card) map[string]any {
	...
	for _, elem := range card.Elements {
		switch e := elem.(type) {
		case agent.CardMarkdown:
			elements = append(elements, map[string]any{
				"tag":     "markdown",
				"content": e.Content,
			})
		case agent.CardDivider:
			elements = append(elements, map[string]any{
				"tag": "hr",
			})
		case agent.CardActions:
			...
			for _, btn := range e.Buttons {
				...
				if e.Layout == agent.CardActionLayoutEqualColumns {
					action["width"] = "fill"
				}
...
// renderCard converts a Card into the Feishu Interactive Card JSON string.
func renderCard(card *agent.Card) string {
